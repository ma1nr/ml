...
function setTypingStatus(isTyping) {
    ...
    statusElement.textContent = isTyping ? 'печатает...' : 'онлайн';

    console.log(`Chat status updated to: ${statusElement.textContent}`);
}
...
async function fetchChatGPTResponse(userInput) {
    ...
    const apiKey = 'sk-dJDLYgERsgO2PN3kdgDOz8cbmnC3aBeu'; // Замените на безопасный метод хранения ключа
    ...
    try {
        const response = await fetch(url, {
            ...
            body: JSON.stringify({
                model: "gpt-4o-mini", // Используйте существующую модель
